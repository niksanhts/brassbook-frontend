...
function AddAlbumModal({ onClose, onAddAlbum }) {
  ...
  const background = useMemo(() => {
    return image ? `url(${URL.createObjectURL(image)})` : '#F4F4F6';
  }, [image]);

  return (
    <Modal onClose={onClose} title="Создание альбома">
      ...
      <Button onClick={() => onAddAlbum({ name, image })} disabled={!name}>СОЗДАТЬ АЛЬБОМ</Button>
    </Modal>
  )
}
